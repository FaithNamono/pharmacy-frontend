    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        _pendingPrescriptions = _prescriptions
            .where((p) => p.status == 'pending' || p.status == 'partially_filled')
            .toList();
...
Future<bool> createPrescription(Map<String, dynamic> prescriptionData, {Uint8List? imageBytes}) async {
  try {
    _isLoading = true;
    notifyListeners();
    ...
    print('📝 CREATE PRESCRIPTION RESPONSE: $response');
...
Future<bool> deletePrescription(int id) async {
  ...
  Future<Map<String, dynamic>?> fillPrescription(int prescriptionId, List<Map<String, dynamic>> items) async {
  try {
    _isLoading = true;
    notifyListeners();

    print('🔄 FILLING PRESCRIPTION: $prescriptionId');
    print('📦 ITEMS TO FILL: $items');
  ...
  Future<String?> downloadPrescription(int prescriptionId) async {
    try {
      final response = await _apiService.get('/api/prescriptions/$prescriptionId/download/');
  ...
  Future<bool> uploadPrescriptionImage(int prescriptionId, String imagePath) async {
    try {
      _isLoading = true;
      notifyListeners();
